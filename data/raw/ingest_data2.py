...
logger = get_logger("Ingest data file")
logger.info(f"Excecution started")
...
logger.info(f"file is created and saved in: {file_path}")
...
logger.info(f"P&L sheet is created successfully")
...
logger.info(f"Cashflow sheet is created successfully")
...
logger.info(f"KPI sheet is created successfully")
...
file_path2 = os.path.join((Path(__file__).resolve().parents[0]), "Financial_data_final_new.xlsx")
...
logger.info(f"All files are merged into one and saved to: {file_path2}")